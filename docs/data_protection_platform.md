...
You can also drill down directly into the associated report from each widget. To do this, click the View full report icon. The report opens with the widget's filters already applied, so it shows the exact scope displayed in the widget. For details on these reports, see [Predefined Veeam ONE Reports](predefined_reports.md).

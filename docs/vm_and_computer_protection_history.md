...
* Job types: defines a list of job types to evaluate in the report (VM backup, Replication, File to tape, Backup to tape, Backup copy, SQL log backup, Oracle log backup, Agent backup policy, Agent backup, File backup, Nutanix AHV backup, Application backup policy, PostgreSQL log backup, Cloud Director backup, Cloud Director replication, SOBR sessions: Offload (copy policy), Offload (move policy), Download, Archiving, Retrieval, Object to tape backup, Object storage backup).

---
title: "Managing Widgets"
product: "vone"
doc_type: "userguide"
source_url: "https://helpcenter.veeam.com/docs/one/userguide/manage_widgets.html"
last_updated: "6/24/2026"
product_version: "13.0.2.6723"
---

# Managing Widgets


You can edit settings of dashboard widgets, create reports based on widgets and remove widgets from dashboards.

In This Section

* [Changing Widget Properties](modify_widgets.md)
* [Creating a Widget Report](create_widget_report.md)
* [Deleting Widgets](delete_widgets.md)


